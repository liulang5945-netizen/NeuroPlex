...
class RecursiveImprover:
    ...
    def analyze_weakness(self, domain_error_rates: Optional[dict] = None,
                         coactivation_stats: Optional[dict] = None) -> List[ImprovementProposal]:
        """
        分析系统弱点，生成带神经新生标记的改进提案。

        策略层职责：识别"哪里弱"+"是否需要新神经元"，不负责创建。
        创建由 sleep_engine 调用 neurogenesis 执行。

        Args:
            domain_error_rates: {domain: error_rate}，来自 feed_engine
            coactivation_stats: 共激活统计（可选，用于检测孤立模式）

        Returns:
            List[ImprovementProposal]，每个提案带 needs_new_neuron 和 target_domain
        """
        proposals: List[ImprovementProposal] = []

        # 1. 基于域错误率分析：高错误率域需要新神经元
        if domain_error_rates:
            for domain, error_rate in domain_error_rates.items():
                if error_rate > 0.5:
                    proposals.append(ImprovementProposal(
                        proposal_type="architecture",
                        description=f"域 '{domain}' 错误率 {error_rate:.0%} 过高，需要新生神经元",
                        old_value=f"error_rate={error_rate:.2f}",
                        new_value=f"为 {domain} 创建新神经元（neurogenesis）",
                        confidence=min(error_rate, 0.95),
                        evidence_count=int(error_rate * 100),
                        needs_new_neuron=True,
                        target_domain=domain,
                    ))
                elif error_rate > 0.3:
                    proposals.append(ImprovementProposal(
                        proposal_type="training",
                        description=f"域 '{domain}' 错误率 {error_rate:.0%} 偏高，需要更多训练数据",
                        old_value=f"error_rate={error_rate:.2f}",
                        new_value=f"增加 {domain} 域训练样本",
                        confidence=0.7,
                        evidence_count=int(error_rate * 100),
                        needs_new_neuron=False,
                        target_domain=domain,
                    ))

        # 2. 基于共激活统计检测孤立模式（可能需要新神经元协同）
        if coactivation_stats:
            isolated = coactivation_stats.get("isolated_neurons", [])
            if isolated:
                proposals.append(ImprovementProposal(
                    proposal_type="architecture",
                    description=f"检测到 {len(isolated)} 个孤立神经元，可能需要新生神经元协同",
                    old_value=f"isolated: {isolated[:5]}",
                    new_value="为孤立模式创建协同神经元",
                    confidence=0.6,
                    evidence_count=len(isolated),
                    needs_new_neuron=True,
                    target_domain="general",
                ))

        # 3. 基于策略记录分析（复用 analyze_and_improve 的逻辑）
        strategy_proposals = self.analyze_and_improve()
        for p in strategy_proposals:
            if not p.target_domain:
                p.target_domain = "general"
        proposals.extend(strategy_proposals)

        logger.info(f"analyze_weakness: {len(proposals)} 个提案，"
                    f"{sum(1 for p in proposals if p.needs_new_neuron)} 个需要神经新生")
        return proposals

    def generate_weakness_training_data(self, domain: str = "general",
                                        weaknesses: Optional[List[str]] = None) -> List[dict]:
        """
        按域生成弱项针对性训练数据。

        将弱点分析结果转化为可训练的样本，按 domain 分类。
        供睡眠引擎 Phase 2 训练对应域的 Cortex 神经元。

        Args:
            domain: 目标域
            weaknesses: 弱点列表（若 None，自动从策略记录推断）

        Returns:
            训练样本列表（每个样本带 domain 标签）
        """
        if weaknesses is None:
            # 从策略记录推断弱点
            weaknesses = self._infer_weaknesses_from_records(domain)
        ...
        for w in weaknesses:
            w_lower = w.lower()
            if "数学" in w or "math" in w_lower:
                samples.extend(self._math_weakness_samples(domain))
            elif "代码" in w or "code" in w_lower or "python" in w_lower:
                samples.extend(self._code_weakness_samples(domain))
            elif "准确" in w or "accuracy" in w_lower or "低" in w:
                samples.extend(self._accuracy_weakness_samples(domain))
            elif "工具" in w or "tool" in w_lower or "react" in w_lower:
                samples.extend(self._tool_weakness_samples(domain))
            else:
                samples.append({
                    "type": "react",
                    "domain": domain,
                    "task": f"请针对以下弱项提供详细解答：{w}",
                    "steps": [{
                        "thought": f"需要针对弱项 {w} 提供训练数据",
                        "action": "query_knowledge",
                        "action_args": {"input": w},
                        "observation": f"（此为自动生成的弱项针对性训练样本，指向：{w}）",
                        "final_answer": f"（此为自动生成的弱项针对性训练样本，指向：{w}）",
                    }],
                })

        logger.info(f"generate_weakness_training_data: 域 '{domain}' 生成 {len(samples)} 条样本")
        return samples

    def _infer_weaknesses_from_records(self, domain: str) -> List[str]:
        """从策略记录推断指定域的弱点。"""
        weaknesses = []
        domain_records = [
            r for r in self._strategy_records
            if domain in r.task.lower() or domain in r.strategy_content.lower()
        ]
        failed = [r for r in domain_records if not r.success]
        if len(failed) > len(domain_records) * 0.3 and domain_records:
            weaknesses.append(f"{domain} 域任务失败率偏高")
        if len(domain_records) < 5:
            weaknesses.append(f"{domain} 域交互数据不足")
        return weaknesses or [f"{domain} 域需要更多训练数据"]

    def _math_weakness_samples(self, domain: str = "math") -> List[dict]:
        """数学弱项训练样本"""
        return [
            {
                "type": "react", "domain": domain,
                "task": "计算 128 × 37 的结果",
                "steps": [{"thought": "分解计算", "action": "calculate", "action_args": {},
                           "observation": "128 × 37 = 128 × (40 - 3) = 5120 - 384 = 4736",
                           "final_answer": "4736"}],
            },
            {
                "type": "react", "domain": domain,
                "task": "什么是勾股定理？请用例子说明",
                "steps": [{"thought": "解释几何定理", "action": "query_knowledge", "action_args": {},
                           "observation": "勾股定理：直角三角形中 a² + b² = c²。例：a=3, b=4 -> c=5",
                           "final_answer": "勾股定理：直角三角形中 a² + b² = c²。例：a=3, b=4 -> c=5"}],
            },
        ]

    def _code_weakness_samples(self, domain: str = "code") -> List[dict]:
        """代码弱项训练样本"""
        return [
            {
                "type": "react", "domain": domain,
                "task": "用 Python 写一个二分查找函数",
                "steps": [{"thought": "实现二分查找", "action": "write_code", "action_args": {},
                           "observation": "def binary_search(arr, target):\n    left, right = 0, len(arr)-1\n    while left <= right:\n        mid = (left + right) // 2\n        if arr[mid] == target: return mid\n        elif arr[mid] < target: left = mid + 1\n        else: right = mid - 1\n    return -1",
                           "final_answer": "def binary_search(arr, target):\n    left, right = 0, len(arr)-1\n    while left <= right:\n        mid = (left + right) // 2\n        if arr[mid] == target: return mid\n        elif arr[mid] < target: left = mid + 1\n        else: right = mid - 1\n    return -1"}],
            },
        ]

    def _accuracy_weakness_samples(self, domain: str = "general") -> List[dict]:
        """准确度弱项训练样本"""
        return [
            {
                "type": "react", "domain": domain,
                "task": "请详细解释相对论的基本原理",
                "steps": [{"thought": "解释物理学概念", "action": "query_knowledge", "action_args": {},
                           "observation": "相对论由爱因斯坦提出，包含狭义和广义两部分。狭义相对论基于光速不变原理和相对性原理……",
                           "final_answer": "相对论由爱因斯坦提出，包含狭义和广义两部分。狭义相对论基于光速不变原理和相对性原理……"}],
            },
        ]

    def _tool_weakness_samples(self, domain: str = "general") -> List[dict]:
        """工具使用弱项训练样本"""
        return [
            {
                "type": "react", "domain": domain,
                "task": "搜索 Python 3.12 的新特性并总结",
                "steps": [{"thought": "使用搜索工具", "action": "search", "action_args": {"query": "Python 3.12 新特性"},
                           "observation": "Python 3.12 引入了更详细的错误信息、类型参数语法改进、per-interpreter GIL 等特性……",
                           "final_answer": "Python 3.12 引入了更详细的错误信息、类型参数语法改进、per-interpreter GIL 等特性……"}],
            },
        ]
    ...
    def design_next_generation(self, current_model_info: dict) -> dict:
        ...
        import warnings
        warnings.warn(
            "design_next_generation 已废弃，神经元架构通过 neurogenesis 动态新增神经元。"
            "请使用 analyze_weakness() + neurogenesis 替代。",
            DeprecationWarning,
            stacklevel=2,
        )
        logger.warning("design_next_generation 已废弃（神经元架构使用 neurogenesis 替代代际迁移）")

# Taiji 算子设计：替代 TransformerBlock 的群体神经元原生底层

> 状态：✅ 纯 Python 原型已实现（[neuroplex/taiji.py](file:///workspace/neuroplex/taiji.py)）+ 算子级回归 21/21 PASS（[verify_taiji_operator.py](file:///workspace/scripts/training/verify_taiji_operator.py)）
> 日期：2026-08-20
> 前置：[BIO_INSPIRED_ARCHITECTURE_PLAN.md](file:///workspace/plans/active/BIO_INSPIRED_ARCHITECTURE_PLAN.md)（群体架构）、[CODE_WIKI.md](file:///workspace/CODE_WIKI.md)（sparsely routed population neural network）
...
| 阶段 | 内容 | 状态 |
|---|---|---|
| ✅ 阶段 0 | 纯 Python 原型 + 算子级回归 | 完成（本轮） |
| ⬜ 阶段 1 | ResonanceNeuron 集成：配置开关 `use_taiji`，ResonanceNeuron 可选装配 TaijiBlock；phase 从 ensemble binding 注入 | 待做 |
| ⬜ 阶段 2 | 集成回归：用 TaijiBlock 跑一轮 ensemble forward，确认共振轮次、field I/O、lm_head 输出形状全部正确 | 待做 |
...
**唯一下一步**：阶段 1（ResonanceNeuron 集成 + phase 注入），冻结现有 9 成员生产权重，新增 `use_taiji` 配置开关默认 False（不影响现有训练），验证 TaijiBlock 在真实 ResonanceNeuron 里能跑通一轮 ensemble forward。
...
- **不动 ensemble 层**：taiji 只替换神经元内部 block，ensemble 的共振编排、STDP、coaction 不变

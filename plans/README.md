...
T5 已完成：20 个单次关联的首四分位与总体保留率均为 100%，memory lesion 后降为 0%，慢参数未变化；但 `cell_0/cell_2` 各承担 20 次写入，`cell_1` 为 0，两个活动细胞还保存了完全相同的副本。当前唯一下一步建议（待架构决策）是加入**跨 episode 持久的细胞本地活动稳态痕迹**，以局部疲劳项抑制固定赢家，再执行 T5-bis；不采用中央路由或随机轮换伪造分工。

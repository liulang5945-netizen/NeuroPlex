...
- 副作用：协作 PPL 79.2 → **372.7**（词表粒度更细 + 73% 新 token 行为子 piece 平均近似初始化）
- **增量微调是词表升级的必要收尾**（372.7 不可用，微调后新 token 语义可学），非新方向

**下一步（唯一）**：增量微调对话数据（`finetune_cross_spec.py --resume --augment`，
50K 新 tokenizer 下自动一致），让 73% 新 token embedding 学习真实语义，恢复/超越 PPL。

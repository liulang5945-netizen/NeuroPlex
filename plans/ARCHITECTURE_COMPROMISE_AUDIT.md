...
**决策时点（2026-08-04 修正：跨域验证路径优先）**：
- **Step 1（进行中）**：当前 zh 综合体 + EOS+短答案重训 → 验证能正常对话
- **Step 2**：加入 code/math/en 等特定能力神经元到已能对话的基底，直观测试跨域涌现（详见 §4.0b 候选 1）
- **Step 3**：若跨域涌现明显 → 当前架构可承载涌现；若跨域无涌现 → 规格阶梯升级或方向 B

**关键认知**：
1. 协作层随神经元数 O(N²) 增长 + **规格升级触发 U 跳跃全局放大**（unified 放大效应，§6.1）
2. **跨域涌现历史已实验过有迹象**，当时因"缺乏能对话的基底"而搁置；现在重训完成后可重新验证（§4.0b 候选 1）

**详见**：[§4.0 架构本源矛盾](#40--架构本源问题协作占比偏低-vs-上限未定论2026-08-04-决策点) | [§4.0b 真实定性+涌现探讨](#40b-当前架构的真实定性--新能力涌现探讨2026-08-04-深化) | [§6 方向 B 备案](#六方向-b-备案小神经元--强协作架构2026-08-04-设计)

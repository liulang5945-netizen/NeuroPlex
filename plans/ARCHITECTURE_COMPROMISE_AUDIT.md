...
**关键认知（2026-08-04 代码查证修正）**：协作层参数随神经元数 + 规格升级双维度增长：
- side_channels：每条 `nn.Linear(field_dim, hidden_size, bias=False)`，**pairwise 全连接 = O(N²)**
- cross_spec 投影层：每个神经元一对（正向+反向）2 层 MLP，**O(N) 线性**
- **更大规格神经元**的 field_dim 和 hidden_size 都更大，单条 side_channel 参数成比例放大

**单条协作通道参数实测**（从代码计算，非估算）：

| 神经元规格 | hidden | field_dim | 单条 side_channel | 单向 cross_spec |
|-----------|--------|-----------|------------------|----------------|
| FOUNDATION | 384 | 4096 | 1.57M | 33.6M |
| COMPACT | 512 | 2048 | 1.05M | 16.8M |
| STANDARD | 768 | 3072 | 2.36M | 25.2M |
| **EXPERT** | **1024** | **4096** | **4.19M** | **33.6M** |

**关键洞察**：EXPERT 规格的单条 side_channel 参数（4.19M）是 COMPACT（1.05M）的 **4 倍**。
引入 EXPERT 神经元会让协作层参数随 N² 增长更陡峭——这正是用户指出的"更大规格神经元 → 协作层扩展规模更大"。

**当前架构的真实增长模型**（单 domain，hybrid 拓扑退化为全连接）：

假设阵容为 (N_c × COMPACT) + (N_s × STANDARD) + (N_e × EXPERT)，N = N_c + N_s + N_e：

| 阵容 | 神经元数 N | side_channels 连接数 | side_channels 参数 | cross_spec 参数 | **协作层总参** | 神经元主体总参 | **协作:主体 比** |
|------|----------|---------------------|------------------|----------------|--------------|--------------|----------------|
| 当前 | 5 (4C+1S) | 20 | 25M | 105M | **130M** | 338M | **0.38:1** |
| +1 EXPERT | 6 (4C+1S+1E) | 30 | 50M | 140M | **190M** | 719M | 0.26:1 |
| +2 EXPERT | 7 (4C+1S+2E) | 42 | 88M | 173M | **261M** | 1100M | 0.24:1 |
| 翻倍 | 10 (8C+2S) | 90 | 146M | 210M | **356M** | 676M | **0.53:1** |
| 20 (16C+4S) | 380 | 653M | 420M | **1073M** | 1.35B | **0.79:1** |
| 20 混合 (10C+5S+5E) | 380 | 980M | 420M | **1400M** | 2.35B | **0.60:1** |
| 50 (40C+10S) | 2450 | 4230M | 1.05B | **5280M** | 3.38B | **1.56:1** |

**修正结论**：
1. 单纯增加 COMPACT 神经元，需要 N≥50 才能让协作层超过主体（1.56:1）
2. **引入 EXPERT 神经元**会让单条 side_channel 参数翻 4 倍，但 EXPERT 本身主体也大（381M），所以**协作:主体比反而可能下降**
3. 真正能让协作层占比快速提升的是**增加神经元数量 N**（O(N²) 增长），而不是单纯升级规格
4. **规格升级的真正价值**：EXPERT 神经元 field_dim=4096，能承载更丰富的场信息，协作的"信息带宽"更高（但参数效率不如增加 N）

...
当前进入 micro 装配方式决策节点。新增验证假设：三个 6.97M 专长成员不依赖 9 成员
生产群体，也许可以作为一个独立小型协作群体工作；架构已有 `active_nids` 部分激活和
`ResonanceEnsemble` 子集装配入口。唯一推荐下一步：先用同一批 current-only、HF-only、
90/10 mixed 训练出的 6.97M 成员，分别完成“三成员独立装配”与“完整装配仅激活三成员”
的 finite forward 和固定生成对照；在该 canary 通过前，不产出生产 checkpoint、不修改 9 成员
默认装配，也不重启 route head 变体。

三成员独立协作 canary 已完成（2026-08-20）：重新以同一随机初始化训练 3 个
`micro_2x128`（实际 local params=6.974593M）成员各 800 步，回评使用 32 条 current 与
32 条 HF 样本，current-only / HF-only / 90/10 mixed 的 PPL 分别为
`1,750.27/3,558.71`、`6,302.75/837.18`、`1,647.23/1,765.91`，专长方向仍清晰。
将三个成员独立放入 field_dim=256 的 `ResonanceEnsemble` 后，3 轮 forward 全部 finite，
三轮 active history 均为 `[3,3,3]`；将同一批成员加入临时 12 成员群体、但只通过
`active_nids` 激活这三个成员时，3072 维生产场的 3 轮 forward 也全部 finite，证明架构的
“独立三成员装配”和“完整装配部分激活”两条路径都成立。仅激活三成员的两条固定生成重复
bigram 为 `0.3548/0.3152`，低于 9 成员基线的 `0.5670/0.6105` 和 12 成员全量的
`0.5778/0.6105`，但文本仍然碎片化，样本量也只有两条，不能宣称智能或群体能力提升；
不写入生产。完整报告为 `reports/micro_specialist_three_only_697m_800_20260820.json`。

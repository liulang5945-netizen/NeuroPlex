...
核心实验：多同域神经元协作能否涌现集体智能？ ✅ 涌现确认！
  ...
  生成质量：PPL达标但贪婪解码输出仍乱（"yx x x...they they...《《"）
    → 根因：domain→general token 转换粗糙（decode→re-encode 丢失对齐），非模型本身
    → 待改进生成管线（forward_train 已验证 PPL 有效，问题在解码回路）

  → 下一步：改进生成管线 / 扩到10神经元 / 混合域联合训练
```
...
1. **连贯基线 + 协作测试**（✅ 完成）→ 联合训练 5 神经元，协作 PPL=24.4 < 个体 47.2
   ...
   - 生成质量待改进（PPL 达标但 token 转换粗糙，根因在解码回路非模型）
2. **Plan C 路由**（已决定，当前可推进）→ 竞争式激活取代分类式路由（L1/L2 均废弃）
   - 协作有效已确认 → Plan C 竞争单元是"聚落"（族长带领集群竞争）= 用户原始思路落地
3. **闭环**（排在 2 之后）→ 睡眠训练 → 更好的神经元 → 更清晰的路由信号
4. **扩展**（排在 3 之后）→ 更多域神经元 + 更大数据规模

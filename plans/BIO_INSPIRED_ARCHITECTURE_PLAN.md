...
核心实验：多同域神经元协作能否涌现集体智能？ ✅ 涌现确认！
  ...
  → 两阶段训练（🔄 阶段二进行中，2026-07-24）：
    ...
    阶段一 ✅ 完成：10 个神经元 × 8000 步/人（每人 ~33min，总 5.5h）
      个体 PPL 6.6~8.0, argmax 平均 73.4%（比联合训练 PPL=11/argmax=68% 显著提升）
    阶段二 🔄 进行中：冻结 backbone(73.4M)，训练协作层
      第一轮 lr=1e-3 ❌ 发散：PPL 3.8→4.8 持续上升，best@step80 后再无改善
        → 诊断：lr 过高 + 残差模式梯度大 → 单步 ce 波动 0.19~3.46
      第二轮 lr=1e-4 🔄 重启中：降低 10 倍 lr，预计更稳定收敛
...
**四大升级已全部实现，两阶段训练（450M tokens + 数据多样性 + 正则化）进行中，目标 argmax 85%+。**
...
| | zh_j0~j9 (10×compact 两阶段训练) | 🔄 阶段二进行中 | 阶段一✅完成(个体PPL6.6~8.0/argmax73.4%)；阶段二🔄协作训练(lr=1e-4 residual模式) |

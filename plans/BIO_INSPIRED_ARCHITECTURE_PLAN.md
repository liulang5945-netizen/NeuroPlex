...
- **hub neuron 设计与实现**（缺口 L，进行中：阶段 1-2 + 阶段 3 全部完成（装配/协作层/锚定+对比 loss），剩余 GPU 正式训练 + 阶段 4 评估）：参考人脑联合皮层，expert 规格 + 256K lm_head + hub-and-spoke 拓扑 + 跨域对比 loss。详见 [HUB_NEURON_DESIGN.md](file:///e:/taiji-neuron/plans/archive/HUB_NEURON_DESIGN.md)

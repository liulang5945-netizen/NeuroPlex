...
核心实验：多同域神经元协作能否涌现集体智能？ ✅ 涌现确认！
  ...
  → 下一步：加大联合训练规模（5神经元×8000步×20K数据，进行中）
    目标：argmax 56%→85%+，验证更多训练能否让生成连贯
```
...
| **神经元** | zh_j0~j4 (联合训练5个) | ✅ 涌现确认 | 协作PPL=24.4 < 最强个体47.2(-48.3%)；3个个体PPL>1000(zh_j3=1.8M)但集体超越最强 |
...
| **🧠 联合训练** | forward_train() + train_cortex_joint | ✅ 完成 | 全可微聚合，5神经元×3000步，协作PPL 24.4 < 个体47.2 |
| **🔍 生成诊断** | argmax/往返/自回归3项测试 | ✅ 完成 | 根因=训练不足(argmax56%)，非管线/非融合；数据23K干净仅用8K |
| **🧠 加大训练** | 5神经元×8000步×20K数据 | 🔄 进行中 | 目标argmax 56%→85%+让生成连贯 |
...
1.5. **生成可用性**（🔄 进行中）→ 加大训练让生成连贯
   - 诊断确认根因=训练不足（argmax 56%，非管线/非融合/非数据）
   - 实验：5神经元×8000步×20K数据，目标 argmax 56%→85%+
   - 若达标 → 态极首次可生成连贯对话；若不达标 → 需更大模型/更多步数

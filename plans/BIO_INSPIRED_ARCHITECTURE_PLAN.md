...
核心实验：多同域神经元协作能否涌现集体智能？ ✅ 涌现确认！
  ...
  → 第二轮联合训练（🔄 进行中，2026-07-24）：
    下载 548K 条中文文本（维基 50万 + alpaca 4.8万）= 131.7M tokens
    数据/参数比 0.006 → 0.268（改善 45 倍）
    正则化：dropout=0.1 + weight_decay=0.1 + WSD 学习率调度
    残差预测编码模式训练（训练+推理配套）
    12000 步，目标 argmax 85%+
  ...
  → 四大架构升级已全部实现（2026-07-24）：
    ...
    下一步：等第二轮训练完成 → 评估 argmax 85%+ → 验证生成连贯
```

### 0.2 架构演进依赖关系

```
生成可用（第二轮训练：131.7M tokens + 正则化 + 残差模式）→ 验证 argmax 85%+ 能否生成连贯
     ↓
四大架构升级（✅ 全部完成 2026-07-24）：
  ...
  ④ 稀疏激活 ✅ → auto_topK自动选择（active_nids='auto_topK'）
     ↓
下一步：第二轮训练完成 → 评估（PPL+argmax+生成）→ 验证残差模式端到端效果
     ↓
...
**四大升级已全部实现，第二轮训练（131.7M tokens + 正则化）进行中，目标 argmax 85%+。**
...
| | zh_j0~j9 (10×compact 第二轮491M) | 🔄 训练中 | 131.7M tokens + dropout + weight_decay + WSD + 残差模式；目标argmax 85%+ |

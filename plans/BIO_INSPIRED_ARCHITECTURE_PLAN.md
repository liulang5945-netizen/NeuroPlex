...
- **剩余对比问题清单（待办）**：
  ...
  - **✅ 培养期端到端闭环验证（2026-08-11，C25-E 后下一阶段：喂养数据渐进改善落地）**
    ...
    - **✅ 渐进改善验证 + 破坏性更新修复（2026-08-11，verify_feed_sleep_progressive.py 24/24 PASS）**
      ...
      - **✅ 平台期定性：容量饱和为主（2026-08-11，verify_feed_sleep_scale.py 11/11 PASS）**
        - **实验**：16 条 held-out 评估集（降噪）+ 3 轮 × 16 条混合主题（48 条，56 条池）对照 progressive（8 条/轮，主题顺序）
        - **数据**：scale 3751 → 1154 → 602 → 569（轮 2→3 仅 -5.6%）；progressive 2161 → 972 → 489 → 384（轮 3 后平台）——**两个独立实验同模式：前 2-3 轮大幅改善后斜率骤降**
      ...
      - **✅ leader 信号 A/B 收益确认（2026-08-11，verify_zh_leader_ab.py）——容量非主要杠杆，转向数据**
        - **实验**：12 zh prompts × 3 采样（temperature 0.55 波动消除），强制 leader=zh_std0(134M) vs 当前机制（场共振分 leader）
        - **结果**：平均长度 42.5 → 47.1（+11%）、主题词命中 0.293 → 0.346（+18%）、字符重复率持平（0.162→0.163）、非空持平（36/36）——但逐 prompt 波动大（B 胜 6 / A 胜 3 / 平 3，注意力机制/梯度下降/词嵌入反而 A 胜）
        - **结论**：强制 134M 仅**轻微占优**，场共振分与生成质量部分脱节但非主要瓶颈——**容量不是 zh 生成的主要杠杆**；结合培养期平台定性（51M 容量饱和）+ 单 neuron 生成全碎片（51M/134M 都碎片），**zh 生成瓶颈 = 对话数据质量**（zh_aug*/zh_std0 共用 C24 dialogue 数据限制）
        - **路径修正**：leader 信号改进降级为可选微优化（收益边际 +11% 且波动）；**主线 = C24 dialogue 数据扩充重训**（对话级数据直接提升 zh_aug*/zh_std0，CPU 数小时量级，需确认数据规模/预算后启动）
      - **✅ dialogue 欠训练根因诊断（2026-08-11，diag_dialogue_data.py + 训练日志）——碎片根因 = 欠训练**
        - **训练历史**：5 个 dialogue neuron（zh_aug0-3/zh_std0）全部**只训练 4000 步**（finetune_neuron_dialogue 显式 --steps 4000，~7s/步 CPU → 7.7h/neuron 预算），best_val_ppl **89-102 全部偏高**（对比 code/math <4）
        - **未收敛证据**：zh_std0 日志 val PPL 持续下降（192@1k → 130@2k → 95@4k）且仍在下行——**训练被预算截断，非收敛平台**
  ...
  - **架构审视（2026-08-11，基于"Transformer vs 人脑 / 超越 Transformer"三轮讨论）——方向记录，未实施**
    - **讨论结论**：① Transformer=联想检索（attention 是 Hopfield 一步检索的退化特例），缺三样：可写记忆/在线学习、具身世界模型、内禀动机；② 超越 Transformer 已被实证的两条换道线=Titans 可写记忆（推理时向记忆写入，打破长上下文瓶颈）+ 中科院瞬悉 2.0 脉冲 SNN（事件驱动，能效/长序列）；③ 下一代基座=多机制拼装而非单一新层
    - **态极定位**：不是"更好的 Transformer"，而是"Transformer 组件化 + 生命周期闭环"的工程先行者——回合级路由（C19，跨脑区是传递非竞争）、多词表+转译（C21）、神经发生四阶段（IntegrateEngine）、共振多轮迭代（C25 连续时间化=多步联想检索）均与讨论结论同向
    - **核心洞察（最重要）**：共振场（4096-dim，推理时可写的共享状态）**已是"可写记忆"的形态**——但缺 ① 写无学习信号（field_write 是池化，非 Titans 式 memory-as-model）② 写后不固化（场随会话消失，不进长时记忆；对比文档 2.1 记忆定义也未含场）→ 态极踩在"可写记忆"门槛上，未进门
    - **方向（上限最高单点，C26 候选）**：把共振场升级为"可学习可固化的记忆层"——场写入加权由学习信号驱动（Titans 式轻量版）+ SleepEngine 增加"场固化"阶段（高频场模式沉淀为记忆向量/神经元权重，突触稳态下调的工程对应）→ 一次打通讨论的"可写记忆"与态极自身的"睡眠巩固"两点，并可能让协作从判定层走向生成层
    - **遗留风险**：协作目前只在判定层（C21/C25 收敛后 leader 用 round1 无场 logits，防污染是正确取舍），"场影响生成"这条上限路径未打通；zh 生成数据瓶颈在前（对话数据质量 > 容量/机制）
  - **✅ C26 场固化最小冒烟（2026-08-11，架构审视"可写记忆第 0 格"落地，verify_c26_field_memory.py 11/11 PASS）**
    - **实现**：`taiji/resonance/field_memory.py`（FieldMemoryBank：固化=L2 归一化+余弦去重（去重阈值 0.92，人脑突触稳态下调的工程简化）、检索=余弦 top-k、持久化=.pt）+ `sleep_engine.py` 挂载（Phase 1.5 场固化：record_field_memory 队列 → 睡眠沉淀 → field_memory.pt；get_field_memory 动态匹配真实场维度 3072）
    - **验证（真实装配 9 神经元）**：睡眠固化 4 条 ✓ / 重复固化去重 added=0 ✓ / **跨会话检索 top-1 全部命中 4/4**（sim 0.45-0.69，think() 返回的 field_state 快照——注意 generate 后 cortex.field 是空的，场快照必须从 think 返回值截获）✓ / 注入管线 4/4 + 注入改变生成输出 4/4 ✓ / 重启恢复（新实例 load 后检索仍命中）✓ / sleep() 主流程已挂载 ✓
    - **机制边界（如实）**：记忆"完整复述进生成"受 zh dialogue 欠训练限制（生成碎片 `<0x0A>1.`），标注为**续训完成后回归项**——检索相似度已足够高（0.45-0.69），机制闭环（写→固化→检索→注入）成立
    - **意义**：讨论结论"态极踩在可写记忆门槛上"的第一格实证落地——共振场可固化、可跨会话检索、可注入生成；后续（C26 增量）方向=可学习写策略（Titans 式 memory-as-model）+ 记忆向量直接条件化 leader 生成（打通"场影响生成"）
    - **对比 Titans 定位评估（2026-08-11 讨论结论）**：态极场记忆完善后**在记忆生命周期维度上可超越 Titans**——Titans 是单层在线记忆（可学习写/读、直接条件化输出），但无巩固、无遗忘选择、无记忆→权重沉淀、无群体协作；态极独有：睡眠巩固（去重已落地 → 修剪/强化）+ 记忆→LoRA 沉淀（海马→皮层两层记忆）+ 群体场空间协作。**最大差距 = 可学习写策略**（Titans 梯度驱动 memory-as-model vs 态极朴素场快照），不补只是"更好的缓存"。完善路径四步：可学习写 → 修剪强化 → 向量条件化生成 → 记忆沉淀

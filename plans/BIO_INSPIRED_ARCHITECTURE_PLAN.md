...
| 记忆（C26） | ✅ 固化+可读+可沉淀+跨重启保留 | FieldMemoryBank + sleep Phase 1.5 + 增量二"记忆向量条件化生成"（verify 10/10）+ **增量三"记忆→突触沉淀（LoRA）"**：高频记忆重放进神经元权重，海马→皮层两层记忆（verify 10/10）+ **装配恢复补**：沉淀 LoRA 随 ckpt 重启装配自动恢复（verify 8/8，rebound=0.0） |

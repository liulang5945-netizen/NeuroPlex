...
核心实验：多同域神经元协作能否涌现集体智能？ ✅ 涌现确认！
  ...
  → 决策点：用户诉求"1B才能正常对话" vs 实验数据"1B反而更差"
    下一步方向待用户确认
```
...
```
模型不完善（个体弱）← 基础，但核心假设就是个体不需要强 ✅已验证
     ↓
协作能否涌现？ ← ✅ 已确认（联合训练 PPL 26.5 < 个体 52.6）
     ↓
...
1.5. **生成可用性**（🔄 进行中）→ 让生成连贯
   ...
   - 5×compact(311M): PPL=24.4，接近连贯基线30，但生成乱码(argmax 56%需85%+)
   - 8×standard(1B): PPL=92.3，比compact差4倍，生成纯乱码——容量非瓶颈
   - 10×compact(491M,不冻结embedding): 训练中，step2600 avg_ce=4.80 PPL=122，趋势优于所有前序实验
   - 核心洞察：CPU上1B无法训到充分；10×compact是最有希望突破的配置
1.6. **field_dim 统一回4096**（已决定，排在生成验证后）→ 消除规格混合限制
   - H9 时代统一 field_dim=4096，P8 改为按比例(compact=2048/standard=3072/expert=4096)
   - P8 按比例引入架构债务：不同规格无法混合(Cortex报错)、需projector才能混合
   - 决定：改回统一4096（compact参数仅+2.8%可忽略），消除规格混合限制
   - 改回后：compact/standard/expert 无缝混合，族长模式(大带小)直接可用，不需要projector
   - 策略：当前10×compact(2048)训练完成验证生成→改回4096重新训练→混合规格接入
2. **Plan C 路由**（已决定，排在生成可用之后）→ 竞争式激活取代分类式路由（L1/L2 均废弃）
   - 协作有效已确认 → Plan C 竞争单元是"聚落"（族长带领集群竞争）= 用户原始思路落地
3. **闭环**（排在 2 之后）→ 睡眠训练 → 更好的神经元 → 更清晰的路由信号
4. **扩展**（排在 3 之后）→ 混合域 + 混合规格(统一4096后) + 更大数据规模

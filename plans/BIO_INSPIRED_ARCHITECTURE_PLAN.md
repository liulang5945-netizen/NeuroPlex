...
唯一推荐下一步：把 `hidden=128 / layers=4 / intermediate=384 / field_dim=512` 作为显式的
实验性小规模规格接入训练入口，运行一个不落盘的短对话 pilot；只比较它自身的有效答案损失、
首 token 和生成可读性，不替换五个 dialogue、不改默认 9 成员阵容，pilot 通过后再接入真实
群体做增益 A/B。

...
```
核心实验：多同域神经元协作能否涌现集体智能？ 🟥 阻塞（0个连贯神经元）
  logit融合(B1)失败 → 族长主导(L)避免崩溃
  4 prompt 整合验证：L3 ≈ S（协作未稳定优于单干）
  摸底审计（2026-07-24）：0个质量好的神经元
    - zh(3MB): PPL=62.4 但模式崩溃("成功的关键"死循环) → 过拟合
    - zh_2(1MB): best PPL~73, 有词汇无语法("能够帮助人们地球测量听听") → 训练不足
    - zh_3(1MB): best PPL~37, 开局有句但迅速退化("我们可以这条...6666") → 训练不足
    - zh_1(1MB): best PPL~48, 输出乱码符号 → 训练不足
    - en/code/math/general: 旧玩具数据, 输出乱码
  贪婪解码诊断：模型真实最佳猜测也是乱的 → 非解码参数问题，是训练不足
  → 结论：族长主导避免融合崩溃，但无法验证协作效果（个体无法"有意义地发放"）
  → 核心困境：人脑单个神经元不能对话但能"有意义地发放"；
    当前神经元连"有意义地发放"都做不到（输出词组堆砌/乱码）
  → 下一步：训练一个 zh 神经元到连贯基线（能产出语法正确的句子）
    这不是让个体"变聪明"，而是让它"有意义地发放"——协作测试的前提
```

### 0.2 三大问题依赖关系

```
模型不完善（个体弱）← 基础，但核心假设就是个体不需要强
     ↓
协作能否涌现？ ← 正在测试（共振场 + MoCo融合）
     ↓
Plan C 竞争路由 ← 已决定方向，排在协作测试之后
     ↓
闭环（睡眠训练 → 更好的神经元 → 更清晰的路由信号）
```

**混乱来源：把三个有依赖关系的问题当成并行问题。**
...
| **神经元** | zh 单干 (3MB) | ❌ 过拟合 | PPL=62.4 但模式崩溃("成功的关键"死循环)，贪婪解码也是重复 |
| | zh_1/2/3 (各1MB best) | ❌ 训练不足 | best PPL 37-73，有词汇无语法，贪婪解码也是词组堆砌 |
| | en/code/math/general | ❌ 旧玩具数据 | 输出乱码，待重训 |
| **🔍 审计** | 摸底诊断（2026-07-24） | ✅ 完成 | 0个连贯神经元；贪婪解码确认非解码问题，是训练不足 |
...
1. **连贯基线**（🟥 当前阻塞）→ 训练至少 1 个能产出连贯句子的 zh 神经元
   - 不是让个体"变聪明"，而是让它"有意义地发放"（语法正确的句子）
   - 人脑单个神经元不能对话，但能有意义地发放；当前神经元连这都做不到
   - 没有连贯基线，协作测试结果不可信（L3≈S 可能只是两个乱码的比较）
2. **协作测试**（排在 1 之后）→ 确认共振场能否涌现集体智能
   - 协作测试的结果决定 Plan C 的"竞争单元"粒度：
     - 协作有效 → Plan C 竞争单元是"聚落"（族长带领集群竞争）= 用户原始思路落地
     - 协作无效 → Plan C 竞争单元只能是单神经元 → 需先让个体够强
   - 跳过协作测试直接写 Plan C，竞争单元粒度会设计错
3. **Plan C 路由**（已决定，排在 2 之后）→ 竞争式激活取代分类式路由（L1/L2 均废弃）
4. **闭环**（排在 3 之后）→ 睡眠训练 → 更好的神经元 → 更清晰的路由信号
5. **扩展**（排在 4 之后）→ 更多域神经元 + 更大数据规模

### 0.5 可推迟的（不阻塞当前实验）

- L2 路由超参调优（已被 Plan C 取代，不再投入）
- 其他域神经元重训（先验证 zh 域协作）

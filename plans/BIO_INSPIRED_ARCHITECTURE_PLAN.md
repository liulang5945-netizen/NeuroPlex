...
模型不完善（个体弱）← 基础，但核心假设就是个体不需要强
     ↓
架构是否完善 ← 取决于"协作能否补偿个体弱" ← 正在测试
     ↓
路由未解决 ← L1 已 86% 够用，Plan C 依赖前面的结果
```
...
| 层 | 组件 | 状态 | 是否阻塞 |
|----|------|------|---------|
| **架构** | Cortex + Ensemble + Field | ✅ 就绪 | ❌ |
| | MoCo logit 融合 | ✅ 就绪 | ❌ |
| | active_nids 参数 | ✅ 刚加 | ❌ |
| **神经元** | zh 单干 (3MB) | ✅ PPL=62.4 | 基准 |
| | zh_1/2/3 (各1MB) | 🔄 训练中 | **关键实验** |
| | en/code/math/general | ❌ 旧玩具数据 | 后续 |
| **路由** | L1 域路由 | ✅ 86% | ❌ 不阻塞 |
| | L2 原型路由 | ⚠️ 43% (支线) | ❌ |
| | Plan C 激活路由 | 🔜 未实现 | 依赖神经元训练 |
| **生命周期** | 睡眠/喂养/调质/新生凋亡 | ✅ 已实现 | ❌ 当前不需要 |

### 0.4 可明确推迟的

- L2 路由优化（L1 够用）
- Plan C 路由实现（需训练充分的神经元）
- 单神经元加数据（如果协作有效，不需要单个变强）
- 其他域神经元重训（先验证 zh 协作）
